...
function install_yuru_fonts () {
  if [ $is_macos ]; then
    ...
    echo 'brew install --cask font-plemol-jp'
    echo 'brew install --cask font-plemol-jp-nf'
    echo 'brew install --cask font-plemol-jp-hs'
    return 0
  fi
...
function install_zsh_shell_utils () {
  ...
  ZSH_NVM_INSTALL_DIR="$XDG_DATA_HOME/zsh/zsh-nvm"
  update_git_history "$ZSH_NVM_INSTALL_DIR" https://github.com/lukechilds/zsh-nvm.git 'master'
  ZSH_DEFER_INSTALL_DIR="$XDG_DATA_HOME/zsh/zsh-defer"
  update_git_history "$ZSH_DEFER_INSTALL_DIR" https://github.com/romkatv/zsh-defer.git 'master'
}
...
pnpm i -g --dangerously-allow-all-builds bun tree-sitter-cli '@11ty/eleventy' json5 '@mermaid-js/mermaid-cli'
